...
- **Cal.com Integration**: Direct API integration with Cal.com for appointment scheduling, configuring two tools (`check_availability`, `book_appointment`) directly within ElevenLabs agents. **Dynamic Schema Detection**: Automatically fetches event type requirements from Cal.com API v2 and builds ElevenLabs tool schemas dynamically based on bookingFields configuration. Supports all Cal.com field types (name, email, phone, text, textarea, number, select, radio, checkbox) with safe fallbacks for unknown types. Includes API key corruption detection to catch database serialization issues early.
