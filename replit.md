...
- **Parent/Child Account System**: Organizations can create unlimited child accounts (sub-accounts) with independent business profiles, integrations, and AI agents. **Account Isolation**: Each child account has its own `business_info` entry with separate API credentials for ElevenLabs, Twilio, and Cal.com. **Account Switching**: Parent accounts can seamlessly switch between their own account and any child account via a dropdown in the navigation. **Implementation**: Uses `parent_account_id` foreign key in the users table, with `getActiveUserId()` helper in the auth hook to prioritize `activeAccountId` from localStorage over the logged-in `userId`. All API calls automatically use the active account ID for complete tenant isolation.
