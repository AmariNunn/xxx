...
- **Parent/Child Account System**: Organizations can create unlimited child accounts (sub-accounts) with independent business profiles, integrations, and AI agents. **Account Isolation**: Each child account has its own `business_info` entry with separate API credentials for ElevenLabs, Twilio, and Cal.com. **Secure Account Switching**: Parent accounts and admins can switch between accounts via POST /api/accounts/switch with server-side session-based authorization. The system validates parent/child relationships and admin permissions before allowing switches. All switch attempts are logged for security audit. Users can reset to their own account via POST /api/accounts/reset. **Implementation**: Uses `parent_account_id` foreign key in users table. Session stores `activeAccountId` for switched accounts. `getActiveUserId()` middleware helper respects account switching by returning `req.session.activeAccountId || req.session.user.id`. All protected endpoints (calls, business info, etc.) use `getActiveUserId()` for complete tenant isolation. Authorization validates switches via `canSwitchToAccount()` which checks admin status or parent/child relationship.
