...
// Get saved notes
router.get("/api/calls/:userId/saved-notes", async (req: Request, res: Response) => {
  try {
    const userId = req.params.userId;
    if (!userId || typeof userId !== 'string') {
      return res.status(400).json({ message: "Invalid user ID" });
    ...
    res.status(200).json({ data: businessInfo.saved_notes || [] });
  } catch (error: any) {
...
// Save a note (max 3)
router.post("/api/calls/:userId/saved-notes", async (req: Request, res: Response) => {
  try {
    const userId = req.params.userId;
    if (!userId || typeof userId !== 'string') {
      return res.status(400).json({ message: "Invalid user ID" });
    ...
    let currentNotes = businessInfo?.saved_notes || [];
    
    // Check if already have 3 notes
    if (currentNotes.length >= 3) {
      return res.status(400).json({ message: "Maximum of 3 notes can be saved" });
    }
    ...
    if (fetchError || !businessInfo) {
      ...
      await supabase
        .from('business_info')
        .update({ saved_notes: updatedNotes })
        .eq('user_id', userId);
    }

    res.status(200).json({ message: "Note saved successfully", data: updatedNotes });
  } catch (error: any) {
...
// Delete saved note by index
router.delete("/api/calls/:userId/saved-notes/:index", async (req: Request, res: Response) => {
  try {
    const userId = req.params.userId;
    const index = parseInt(req.params.index);
    ...
    if (error || !businessInfo || !businessInfo.saved_notes) {
      return res.status(404).json({ message: "No saved notes found" });
    }

    const currentNotes = businessInfo.saved_notes;
    ...
    const updatedNotes = currentNotes.filter((_: any, i: number) => i !== index);
    ...
    res.status(200).json({ message: "Note deleted successfully", data: updatedNotes });
  } catch (error: any) {
